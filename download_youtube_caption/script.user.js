...
var makeTimeline = function (time) {
  ...
  time_array.push(
    Math.floor(time - (time_array[1] * 60 + time_array[0] * 60 * 60)),
  );
...
function loadCaption(selector) {
  ...
  GM_xmlhttpRequest({
    method: "GET",
    url:
      "http://video.google.com/timedtext?hl=" +
      caption.lang_code +
      "&lang=" +
      caption.lang_code +
      "&name=" +
    ...
    onload: function (xhr) {
      if (xhr.responseText !== "") {
        var caption,
          ...
          captions = new DOMParser()
            .parseFromString(xhr.responseText, "text/xml")
            .getElementsByTagName("text"),
          textarea = document.createElement("textarea"),
          output_format =
            FORMATS[document.getElementById(FORMAT_SELECTOR_ID).value] ||
            FORMATS["srt"],
          srt_output = "";
        ...
        for (var i = 0, il = captions.length; i < il; i++) {
          ...
          if (0 <= previous_start) {
            textarea.innerHTML = captions[i - 1].textContent
              .replace(/</g, "&lt;")
              .replace(/>/g, "&gt;");
          ...
          textarea.innerHTML = caption.textContent
            .replace(/</g, "&lt;")
            .replace(/>/g, "&gt;");
          if (output_format === FORMATS["text"]) {
            srt_output += textarea.value + "\n";
        ...
        GM_openInTab(
          "data:text/srt;charset=utf-8," + encodeURIComponent(srt_output),
        );
      } else {
